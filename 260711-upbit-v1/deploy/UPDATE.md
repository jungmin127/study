...
단, 같은 `run_id`라도 로컬에서 "최신 데이터로 갱신" 버튼을 눌러 내용이 더 최신이 된
경우에는 건너뛰지 않고 서버 쪽 결과를 그 최신 내용으로 교체한다(`created_at` 비교).

실행이 끝나면 "백테스트 결과 병합 완료: 신규 N건 추가, 갱신 K건 교체, 기존 M건
건너뜀"이 출력된다. 이후 서버 프론트엔드의 백테스트 목록에서 새로 옮겨진 결과를
확인하고, 그 결과 상세 페이지의 "실거래 전환" 버튼으로 라이브 전략을 만들 수 있다.

로컬 백엔드를 켜둔 채로 실행해도 되지만, grid search가 돌고 있거나 "최신 데이터로
갱신" 버튼을 누른 바로 그 순간처럼 `data/backtest_results.db`에 실제로 쓰기가
진행 중일 때는 피한다 — 단순 목록/상세 조회(읽기)는 안전하다.

지금 25개 결과 전부를 옮긴 뒤 그중 필요 없는 것만 라이브 프론트엔드에서 개별
