...
INDICATOR_CATALOG: list[dict] = [
    ...
    },
    {
        ...
        "description": "포지션 진입 후 도달한 최고 수익률(%)을 추적해, 그 값이 임계값(계단 폭)의 "
            "배수 이상이 될 때마다 손절선을 한 단계씩 끌어올립니다. 손절 용도가 아니라 이미 난 수익을 "
            "지키는 용도라, 최고 수익률이 아직 첫 계단도 못 넘었으면 이 조건 자체가 완전히 "
            "비활성화됩니다(원금 손실 구간에서는 발동하지 않으므로, 손절이 필요하면 같은 조건그룹에 "
            "STOP_LOSS_PCT를 반드시 함께 걸어야 합니다). 임계값은 계단 폭(step)이며, 0 이하로 "
            "설정하면 이 조건 자체가 항상 비활성화됩니다. 활성화된 뒤의 실제 손절선은 "
            "floor(최고수익률/step - 1)*step(항상 0% 이상)으로 계산됩니다.",
        "example": "임계값(step) 5를 넣으면: 최고수익률이 아직 +5% 미만이면 이 조건은 비활성 "
            "상태입니다. +5% 이상이면 손절선 0%(본절), +10% 이상이면 손절선 +5%, +15% 이상이면 "
