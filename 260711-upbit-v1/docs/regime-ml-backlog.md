...
발견** — GaussianHMM 적합이 `END=datetime.now()`로 매 실행 조금씩 자라나는
데이터 창에 민감해 서로 다른 로컬 최적해로 수렴한 것으로 보이며, 두 실행
모두 결론(미채택)은 동일하나 아래 절은 최종 재실행 수치로 갱신했다.
...
`random_state=42`로 고정돼 있어 **같은 데이터**에 대해서는 결정적이지만, 이
스크립트의 `END=datetime.now(timezone.utc)`가 실행 시각마다 조금씩 자라나는
캔들 데이터 창을 만들어내고, GaussianHMM의 EM 적합이 그 미세한 차이에
민감하게 반응해 서로 다른 로컬 최적해(따라서 다른 state 경계)로 수렴한
것으로 보인다 — 같은 실행 안에서 두 번 연속 재실행하면 완전히 동일한
결과가 나오는 것으로 확인했으므로(비결정성이 아니라 데이터 창 변화가
원인), 코드 버그는 아니지만 이 사후 분석 자체가 "실행 시점에 따라 결과가
달라지는" 방법론적 한계를 갖고 있다는 뜻이다. 아래 표는 전부 이번
최종리뷰 대응 재실행(가장 최신) 기준 실측값이다.
...
**마켓별 분리 비교(최종리뷰 지적사항 반영 — 신규)**: 마켓별 HMM은 독립적으로
