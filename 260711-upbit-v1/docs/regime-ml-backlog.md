# 장세 판별 ML — 잔여 작업 백로그 (2026-09-01 갱신 10)
...
**실측 결과(20마켓 실데이터)**: 1차 OOF 전체 n=387,690, 후보(proba_down>=0.5)
n=162,252(전체의 약 42%), 메타 학습 n=113,576/메타 테스트 n=48,676(시간순
70/30 분할).

| threshold(메타) | precision(1차+메타) | recall(1차+메타) |
|---|---|---|
| 0.30 | 0.370 | 0.852 |
| 0.50 | 0.375 | 0.511 |
| 0.70 | 0.350 | 0.148 |
...
**비교(1차만, threshold=0.70)**: precision=0.390, recall=0.108.

**결론 — 메타 게이트가 threshold 튜닝을 못 넘는다**: 메타모델 자체의
threshold를 0.30부터 0.90까지 전부 스윕해도 precision이 **0.313~0.375
사이에서 사실상 평평**하다 — 메타 학습 구간의 기저율(양성 비율
43,181/113,576≈38.0%)과 거의 같은 수준으로, threshold를 올려도(더 확신
있는 예측만 남겨도) precision이 거의 안 오른다는 뜻이다. 이는 메타모델이
"1차의 하락 판정이 진짜 맞는지"를 판별할 실질적인 신호를 74개 피처에서
전혀 못 찾았다는 의미 — 정상적으로 작동하는 메타 레이블링이라면 threshold를
높일수록 precision이 뚜렷이 올라가야 한다(가장 확신 있는 예측만 남기므로).
**어떤 threshold에서도 1차 단순 threshold 튜닝의 precision(0.390, threshold
=0.70)조차 넘지 못했다.**

**해석**: c-2(LightGBM≈LogisticRegression)와 horizon 그리드서치(60~96시간
평평)에 이어, 메타 레이블링(질적으로 다른 접근, kappa가 아니라 precision을
직접 겨냥)까지 실패하면서, **"이 74개 피처로는 어떤 각도에서 접근해도
장세 방향 예측에 유의미한 추가 신호를 뽑아낼 수 없다"**는 결론이 이번
세션에서 시도한 4개 방향(피처 추가/모델 교체/horizon 조정/메타 레이블링)
전부에서 일관되게 나왔다. 근본 원인은 엔지니어링 문제가 아니라 데이터
자체의 신호대잡음비 한계로 보인다.
