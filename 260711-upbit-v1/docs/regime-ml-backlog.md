...
양수(86.2%/984.2%/678.9%)라 지도학습 baseline의 "하락(총기여 -17.7%) vs
하락아님(총기여 +1767.1%)" 같은 방향성 대비가 나타나지 않아 미채택,
5연속 실패로 이 세션의 구조 재검토 시도는 종료하고 다음은 AWS 배포·실전
활용 전환만 남음.
...
을 내는 명확한 방향성 대비였던 반면, HMM 3개 상태는 전부 총수익기여가
양수(86.2%/984.2%/678.9%)이고 승률도 전부 60% 이상(state 0의 100%는 거래수
44건뿐인 소표본)이라 "잃는 상태 vs 버는 상태"를 가르는 신호가 나타나지
않았다. 상태 프로파일 상으로도 두 변수(수익률/변동성)가 주로 변동성 크기로
갈릴 뿐, 방향성(수익 vs 손실)을 가르지 못했다. 5연속 실패(피처/모델/
horizon/메타 레이블링/HMM)로 이 세션의 구조 재검토는 종료하고, 다음은
AWS 배포·실전 활용 전환(백로그 (b) 후보)만 남음.
