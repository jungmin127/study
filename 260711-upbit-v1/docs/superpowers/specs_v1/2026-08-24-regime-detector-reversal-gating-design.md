...
기존 판별기(`docs/superpowers/specs_v1/2026-08-23-realtime-regime-detector-design.md`)는
...
`scripts/regime_backtest.py`가 기존 버그(VALIDATION_START 2024-01-01~현재 범위가
MAX_CANDLES=20,000 초과, 이 스펙과 무관한 기존 이슈)로 실행 불가능해, 최근 800일로
좁힌 임시 검증 스크립트(커밋 안 함, 계획 Task 0/7 전용)로 변경 전/후를 비교했다.
KRW-BTC/ETH/XRP, minutes60, 2024-06-15~2026-08-24.

| 마켓 | 상관계수(전→후) | 정반대 오분류(전→후) | 급상승 hit-rate(전→후) | 급하락 hit-rate(전→후) |
|---|---|---|---|---|
| KRW-BTC | 0.043 → 0.037 | 5 → 11 | 2.0% → 3.8% | 0.8% → 1.0% |
| KRW-ETH | -0.005 → -0.003 | 1 → 5 | 2.5% → 4.7% | 0.0% → 0.3% |
| KRW-XRP | -0.001 → -0.004 | 11 → 9 | 4.8% → 2.7% | 0.0% → 0.3% |
| **합계** | 사실상 무변화 | **17 → 25 (악화)** | 평균 3.1%→3.7%(소폭개선) | 평균 0.27%→0.53%(소폭개선) |

**결론: 1단계(규칙기반 반전 게이팅)는 이번 스펙의 핵심 목표(정반대 오분류 감소)를
달성하지 못했다 — 오히려 3개 마켓 합계 기준 악화됐다(17건→25건).** 극값 카테고리
자체의 hit-rate는 소폭 개선됐고 급상승/급하락이 통계적으로 도달 불가능해지는
과도한 게이팅 회귀도 없었지만, 상관계수는 사실상 변화가 없다.

구현 자체(engine/regime_features.py 5개 함수, regime_detector.py 통합)는 각 태스크
리뷰에서 수식·causality·경계값을 전부 검증받았고 결함이 없다 — 이 결과는 코드 버그가
아니라, "거래량 확인 + VPIN 쏠림 + 지지저항 근접" 조합이 실제 코인시장 반전을 유의미하게
선행 감지하지 못한다는 실증적 신호로 해석해야 한다. 가능한 원인:
- half_life_bars=1일 기반 raw_score가 극값에 도달할 정도로 크면 이미 softmax가 포화
  구간(|score|>0.35)에 들어가 있어, gate(최소 0.3배)로 감쇠해도 argmax 카테고리 자체가
  잘 안 바뀐다 — 감쇠가 카테고리 경계 부근의 애매한 사례에만 실효성 있게 작동하고,
  정작 "정반대로 확신하는" 깊은 포화 사례에는 거의 영향을 못 준다.
- VPIN(거래량 버킷 기반 매수/매도 추정)과 저항/지지 근접이 동시에 성립하는 시점이
  실제 반전 직전과 충분히 강하게 상관돼 있지 않을 수 있다.

**정정(최종 브랜치 리뷰, 2026-08-24): 아래 "안전하다"는 원래 문구는 유일한 실제
호출자에 대해 사실과 다르다.** `backend/regime_service.py`의 `evaluate_market()`이
쓰는 `upbit_data_service.get_candles()`는 `_CANDLE_COLUMNS`(volume/trade_value/
high/low 전부 포함)를 **항상** 반환하므로(`upbit_data_service.py:19`), 하위호환
폴백(컬럼 없을 때 raw_score 그대로)은 `/api/v1/regime/*` → `/regime` 대시보드
경로에서 **한 번도 발동하지 않는다** — 이 경로는 100% 조정 경로를 탄다. 즉 이
브랜치를 머지하면 대시보드의 예측·confusion matrix·현재예측카드 출력이 위 표만큼
**실제로 바로 바뀐다**(그것도 핵심 지표가 나빠지는 방향으로). 되돌리려면
`engine/regime_detector.py`의 `_ADJUSTMENT_COLUMNS` 분기를 끄면 되고, 그때 출력은
변경 전과 byte-identical로 복귀한다(이 안전장치 자체는 유효하다 — "지금 화면이
자동으로 그대로 유지된다"가 틀렸을 뿐).

**추가로 밝혀진 것(최종 리뷰, 실측)**: `adjusted_score = raw_score × volume_confirm ×
reversal_gate`에서 `reversal_gate`(감쇠, 위 원인 분석의 대상)뿐 아니라
`volume_confirm`(최대 1.3배 **증폭**)도 같은 곱셈에 섞여 있고, 이번 검증은 둘을
분리하지 않았다(ablation 없음) — 정반대 오분류 악화(17→25)에 `volume_confirm`의
증폭 효과가 기여했을 가능성이 원인 후보에서 빠져 있었다. 또한 반전 시나리오
회귀테스트를 시드 0~49로 재실행한 결과 `reversal_gate`가 raw_score 자체를 낮추는
것(32/50)과 별개로, `_softmax_categorize`의 포화 구간(|score|>0.35에서 확신도가
0.8524로 고정) 때문에 그 감쇠가 실제 confidence 하락으로 드러나는 경우는 6/50(12%)
뿐이었다 — "포화 때문에 게이팅이 안 먹힌다"는 원인 가설의 정량적 근거다.

**다음 단계는 사용자와 논의 후 결정** — 후보: (a) 게이팅 강도/조건을 재튜닝,
(b) `volume_confirm`/`reversal_gate`를 각각 끄고 측정하는 ablation으로 어느 항이
악화에 기여했는지 먼저 분리 진단, (c) 이 접근 자체를 재고하고 브레인스토밍에서
논의했던 2단계(ML 보조모델)로 조기 전환, (d) 다른 피처 조합 시도, (e) 머지 보류
또는 `_ADJUSTMENT_COLUMNS` 분기를 임시로 꺼서 대시보드를 이전 동작으로 되돌린 채
다음 방향을 정함.
