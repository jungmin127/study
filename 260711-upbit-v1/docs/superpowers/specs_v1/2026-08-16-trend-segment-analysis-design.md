...
3. **횡보 병합**: ZigZag 레그는 정의상 개별 레그 자체가 이미 `threshold_pct` 이상 움직여야 확정되므로, "개별 레그 크기"로는 저변동 구간을 판별할 수 없다. 대신 레그를 순서대로 훑으며 현재 묶음의 시작가 대비 각 레그 종료가까지의 **누적 순변화율**이 `threshold_pct` 미만인 동안 계속 묶는다(=순방향 진행 없이 등락만 반복되는 구간을 횡보로 병합). 단, 어느 레그의 **개별 크기**가 `threshold_pct * SIDEWAYS_LEG_CAP_RATIO`(기본 1.5배) 이상이면 그 레그는 흡수하지 않고 새 추세 레그로 분리한다 — 강한 단일 돌파가 앞뒤 잔물결과 뭉뚱그려져 횡보로 오분류되는 것을 막기 위함이다. 완벽한 추세/되돌림 구분은 아니며(예: 뚜렷한 상승 후 애매한 크기의 되돌림이 이어지면 횡보로 오분류될 수 있음), 실제 결과를 보고 튜닝 가능한 v1 휴리스틱으로 채택한다.
