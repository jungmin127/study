...
[[2026-08-23-realtime-regime-detector-design]]에서 구축한 규칙기반 EWMA 판별기(`engine/regime_detector.py`)는 상관계수가 3개 마켓 전부 `|r|≤0.06`으로 예측력이 거의 없다. 이를 개선하려고 규칙기반 정교화(반전 게이팅, `docs/superpowers/specs_v1/2026-08-24-regime-detector-reversal-gating-design.md`)를 시도했으나, 실측에서 핵심 목표(정반대 오분류)가 오히려 악화되어 push하지 않고 로컬에만 보관 중이다(로컬 main 15커밋, `51d4d59..c8df45a`).
