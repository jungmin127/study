...
- 프론트엔드에는 자동 테스트가 없다(기존 프로젝트 관례) — 각 프론트 태스크의 검증은
  ...
  **`npm run build`는 쓰지 않는다** — 개발 중인 `npm run dev`의 `.next` 캐시를 깨뜨리는 알려진
  문제가 있다.
- 이 플랜은 계획 단계에서 스펙(`docs/superpowers/specs/2026-08-15-trading-journal-dashboard-design.md`)의
...
**Files:**
- Modify: `docs/superpowers/specs/2026-08-15-trading-journal-dashboard-design.md`
...
`docs/superpowers/specs/2026-08-15-trading-journal-dashboard-design.md`의 "엣지 케이스"
섹션에서 다음 항목을 찾는다:
...
```bash
git add docs/superpowers/specs/2026-08-15-trading-journal-dashboard-design.md
