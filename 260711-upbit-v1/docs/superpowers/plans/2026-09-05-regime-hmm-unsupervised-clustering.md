...
**Goal:** `docs/superpowers/specs/2026-09-05-regime-hmm-unsupervised-clustering-design.md`에서 승인된 설계대로, 지도학습(Triple Barrier + LightGBM) 없이 HMM 비지도 클러스터링만으로 뽑아낸 잠재 상태가 실전 매매 성과와 상관이 있는지 사후(ex-post) 분석으로 확인한다.
