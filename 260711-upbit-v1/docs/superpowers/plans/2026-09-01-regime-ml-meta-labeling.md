...
**Goal:** `docs/superpowers/specs/2026-09-01-regime-ml-meta-labeling-design.md`에서 승인된 설계대로 `run_training()`에 `collect_oof` 확장점을 추가해 1차 모델의 out-of-fold 예측을 노출하고, 그 위에 메타 레이블링(2차 이진분류기) 학습+측정 스크립트를 추가해 threshold 튜닝만으로는 못 넘은 precision 장벽(threshold=0.70에서도 39%대)을 메타 게이트로 넘을 수 있는지 실측한다.
