...
**다음 단계는 사용자와 논의 후 결정** — 후보: (a) 게이팅 강도/조건을 재튜닝, (b) 이
접근 자체를 재고하고 브레인스토밍에서 논의했던 2단계(ML 보조모델)로 조기 전환,
(c) 다른 피처 조합 시도. 이번 1단계 코드는 하위호환(컬럼 없으면 raw_score와 완전
동일)이 보장되므로 되돌리지 않고 유지한 채로 다음 방향을 정해도 안전하다.
