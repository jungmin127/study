...
- 교체는 `market`, `current_capital`, `risk_config_json`, `baseline_qty`, `status`(및 `approved_at`/`started_at`/`stopped_at`)를 건드리지 않는다. 새 백테스트 값으로 덮어쓰는 필드는 `source_run_id`, `timeframe`, `buy_conditions_json`, `sell_conditions_json`뿐이다 — `docs/superpowers/specs_v1/2026-08-18-live-strategy-swap-design.md`의 확정 사항.
