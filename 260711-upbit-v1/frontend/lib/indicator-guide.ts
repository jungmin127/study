...
export const INDICATOR_GUIDE: Record<string, IndicatorGuideText> = {
  ...
  TRAILING_STOP_STEP_PCT: {
    meaning:
      '캔들 지표가 아니라 보유 포지션이 진입 후 지금까지 도달한 "최고 수익률(%)"을 기준으로, 그 최고 수익률이 임계값(계단 폭)의 배수 이상이 될 때마다 손절선이 한 단계씩 따라 올라가는 지표입니다. STOP_LOSS_PCT처럼 손절선이 고정돼 있지 않습니다.',
    params: [],
    formula: '손절선(%) = (floor(최고 수익률 ÷ step) − 1) × step  (step = threshold, 단 계산값이 음수면 None = 비활성)',
    thresholdExample:
      '매도 조건 전용(sellOnly), 연산자 "≤" 고정. threshold는 계단 폭(step, 보통 양수, 예: 5)입니다. step을 0 이하로 넣으면 이 조건 자체가 항상 비활성화됩니다. 최고 수익률이 아직 첫 계단도 못 넘었으면 이 조건은 완전히 비활성화됩니다(손절 용도가 아니라 수익 확보 전용이라, 원금 손실 구간에서는 발동하지 않습니다 — 이 조건 하나만 걸어두면 손실 구간에서 아무런 방어 수단이 없으므로, 손절이 필요하면 같은 조건그룹에 STOP_LOSS_PCT를 반드시 함께 걸어야 합니다). 예: step=5, 최고 수익률 +16%면 손절선은 +10% — 수익률이 +10% 이하로 떨어지는 순간 매도. 최고 수익률이 아직 +5% 미만이면(첫 계단 미달) 이 조건은 매도를 일으키지 않습니다.',
  ...
  VPIN: {
    meaning:
      '거래량 버킷(누적 거래량이 일정량에 도달할 때마다 하나씩 완성되는 구간) 단위로, 매수 주도 거래와 매도 주도 거래의 불균형 정도를 0~1 값으로 나타냅니다. 1에 가까울수록 그 구간 거래가 한쪽으로 강하게 쏠렸다는 뜻이며, 급등락 직전의 정보거래(독성 주문흐름) 징후로 흔히 해석합니다.',
    params: [{ key: 'period', role: 'VPIN을 계산할 때 평균 낼 최근 거래량 버킷 개수. 작을수록 값이 더 빠르고 크게 출렁입니다.' }],
    formula:
      '체결 틱의 매수/매도 라벨이 아니라, 거래량 버킷의 가격 변화를 표준화(z)해 표준정규분포 누적함수(Φ)에 넣어 매수 비율을 확률적으로 추정합니다(Bulk Volume Classification). 매수추정량 = 버킷거래량 × Φ(z), 매도추정량 = 버킷거래량 − 매수추정량. 최근 period개 버킷의 |매수추정량−매도추정량|/버킷거래량 평균이 VPIN 값입니다.',
    thresholdExample: '값은 0~1 범위입니다. 예: period=20, 임계값 0.55, 연산자 ">"면 최근 20개 거래량 버킷 동안 주문흐름 불균형이 뚜렷한 구간을 포착합니다.',
