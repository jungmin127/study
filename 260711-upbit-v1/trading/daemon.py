...
async def _run_risk_exit_loop(strategy_id: str, lock: asyncio.Lock | None = None) -> None:
    ...
    경우도 "취소 + 재주문 한 사이클"일 뿐 중복/충돌 주문이 아니다. 6라운드 보강 —
    그 정리 단계가 남아있는 매도 주문의 생사를 확인하지 못하면(취소 실패인데 주문은
    여전히 살아있음, 5xx/네트워크 장애로 조회 자체 실패) exit_for_risk()는 이제
    조용히 진행하지 않고 예외를 올린다. 아래 tick try/except가 그걸 로그로 남기고
